  ...
  for my $uri( @{ $config->{board} } ){
    if( my $res = URI::Fetch->fetch( $uri ) ){
      if( my $content = $res->content ){
        $class->logit("content: %s %s. (%s)", $uri, $uri =~ /^file/o ? 'loaded' : 'retreived', format_bytes length $content);
...
sub content2chunk {
  ...
  my @chunk;
  if( $content =~ /<meta charset=/s ){
    # new format
    my @data;
    $content =~ s{<dd>(.+?)</dd>}{ push @data, CGI::unescapeHTML($1) }egsio;

    for my $data( @data ){
      my $seen = {};
      for my $chunk( split /<br>\n*/o, $data ){
...
sub calc_count {
  ...
  $class->logit("total %s videos.", &commify(scalar @result));
